...
if (( ($in_bytes <  1000 ) and ($out_bytes < 1000)) ) {
  $state = 'WARNING';
  $output = "ALERTA - TRAFICO MINIMO -  entrada: ($in_bytes),  salida: ($out_bytes) ";
}

elsif ( ( $in_ave_pct < $warn_usage ) and ( $out_ave_pct < $warn_usage ) ) {
	$state = 'OK';
	$output =
	"$state - Average IN: "
	  . $in_ave . $suffix . " (" . $in_ave_pct . "%)" . $in_bytes  ." , " 
